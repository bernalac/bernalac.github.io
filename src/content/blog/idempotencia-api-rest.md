...
Una petición puede procesarse correctamente en el servidor y, justo después, perderse la respuesta por un problema de red. Desde el punto de vista del cliente, la operación parece haber fallado. ¿Qué hará entonces? Probablemente reintentarlo.
...
El servidor sí ha creado el pedido. El cliente, sin embargo, no ha recibido confirmación. Desde su perspectiva, no puede saber si la operación se ha ejecutado o no, así que decide reintentar la petición
...
El cliente solo quería crear un pedido. Sin embargo, se han creado dos.
...
En una API REST usamos `POST` para crear recursos o iniciar nuevas operaciones:
...
y el cliente recibe un timeout, no sabe si el servidor llegó a procesar el pago o no. Necesitamos un mecanismo que permita distinguir _"Quiero realizar un pago nuevo"_ de _"Quiero volver a enviar el mismo pago porque no recibí la respuesta"_. Ahí entra `Idempotency-Key`, una clave que permite identificar que varias peticiones pertenecen a la misma operación lógica, aunque sean reintentos de la petición original.
...
Si una petición llega mientras la primera sigue procesándose, debemos tener una política explícita para el estado `PENDING`, por ejemplo, devolver un `409 CONFLICT`.
...
A primera vista parece razonable: primero comprobamos si la clave ya existe y, si no existe, procesamos la operación y guardamos el resultado.

El problema es que la comprobación y la ejecución no forman una **operación atómica**.
...
Para evitar problemas con las llamadas internas a métodos `@Transactional`, podemos separar el flujo en varios servicios.
...
public class OrderService {
        ...
        public OrderService(IdempotencyService idempotencyService, OrderTransactionService orderTransactionService) {
        ...
        public OrderResponse createOrder(String idempotencyKey, CreateOrderRequest request) {
                ...
                boolean reserved = idempotencyService.tryReserveKey(idempotencyKey, requestHash);

                if(!reserved) {
                        return idempotencyService.handleExistingKey(idempotencyKey, requestHash);
                }

                try {
                        OrderResponse response = orderTransactionService.processOrder(request);
                        idempotencyService.completeKey(idempotencyKey, response);
                        return response;
        ...
        private String hash(CreateOrderRequest request) {
                // Implementar método hash que queramos
        }
}
```
...
public class IdempotencyService {
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW) 
		...
		try {
			IdempotencyKeyEntity entity = new IdempotencyKeyEntity(idempotencyKey, requestHash, IdempotencyStatus.PENDING, Instant.now());
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW) 
		...
		IdempotencyKeyEntity entity = repository.findById(idempotencyKey).orElseThrow();
		entity.setStatus(IdempotencyStatus.COMPLETED);
		entity.setResponse(toJson(response));
		entity.setHttpStatus(HttpStatus.CREATED.value());
	...
	@Transactional(readOnly = true) 
		...
		IdempotencyKeyEntity existing = repository.findById(idempotencyKey).orElseThrow();

		if(!existing.getRequestHash().equals(requestHash)) {
			throw new IdempotencyConflictException("La Idempotency-Key ya se utilizó " + "con una petición diferente");
		}
		...
		return switch(existing.getStatus()) {
			...
			case FAILED - >
			throw new OperationFailedException(idempotencyKey);
		};
	}
	...
	private OrderResponse fromJson(String response) {
		try {
			return objectMapper.readValue(response, OrderResponse.class);
		} catch (JsonProcessingException e) {
			throw new IllegalStateException("No se pudo recuperar la respuesta", e);
		}
	}
}
```
...
Representar explícitamente el estado de la operación ayuda a decidir qué hacer en cada caso:
...
La clave ha sido reservada, pero la operación todavía no está marcada como completada.

Puede existir otra petición procesando la operación en ese momento, pero también podemos encontrarnos con un `PENDING` huérfano si la aplicación se detuvo antes de actualizar el estado.
...
- Si los registros expirados se eliminan físicamente o se conservan por motivos de auditoría.
...
Cuando el proveedor no ofrece idempotencia, el problema es más complejo y suele requerir patrones adicionales.
...
| `PENDING`      | La operación sigue en curso         | `409 Conflict`                        |
...
- Si llamas a un proveedor externo, utiliza su mecanismo de idempotencia cuando exista y contempla una estrategia de recuperación o reconciliación cuando no exista.
...
La idempotencia puede parecer un detalle innecesario cuando todo funciona bien: la petición llega, se procesa y la respuesta vuelve al cliente. El problema aparece en cuanto entran en escena redes poco fiables, timeouts, balanceadores y sistemas distribuidos, donde no podemos asumir que cada petición se procese exactamente una vez.
